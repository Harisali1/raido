...
class VehicleImageController extends Controller
{
    ...
    public function index(Vehicle $vehicle)
    {
        ...
        return view('vendor.driver.vehicle-image.index', compact('vehicle', 'vehicleImages'));
    }
    ...
    public function store(Request $request, Vehicle $vehicle)
    {
        ...
        return $request->wantsJson()
            ? new Response('', 201)
            : redirect()->route('vendor.driver.vehicle-image.index', $vehicle);
    }
    ...
    public function destroy(VehicleImage $vehicleImage)
    {
        try {
            ...
            return request()->wantsJson()
                ? new Response('', 201)
                : redirect()->route('vendor.driver.vehicle-image.index');
