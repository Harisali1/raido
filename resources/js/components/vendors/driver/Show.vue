...
export default {
    ...
    methods: {
        ...
        submit: function () {
            ...
            axios.post(`/vendor/driver/update_profile/${this.item.id}`, this.buildFormData())
                .then(() => {
                    this.$alertify.success("Profile Picture Updated Successfully!");
                    setTimeout(() => {
                        window.location.href = `/vendor/driver/${this.item.id}`;
                    }, 1000);
                })
                .catch(err => {
                    this.isActive = false;
        ...
        changePassword: function () {
            ...
            this.isActive = true;
            this.submitting = true;
            axios.post(`/vendor/driver/updatePassword/${this.item.id}`, this.password)
                .then(() => {
