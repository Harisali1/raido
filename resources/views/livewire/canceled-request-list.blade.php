                    ...
                    <div class="portlet-body">
                        ...
                        @if(count($canceledRequests) > 0)
                            <table class="table table-striped table-bordered responsive table-hover table-checkable order-column"
                                ...
                                <tbody>
                                @foreach($canceledRequests as $key => $userRequest)
                                    <tr class="odd gradeX">
                                        <td> {{ $key +1 }}</td>
                            ...
                            @if($canceledRequests->total() > $canceledRequests->perPage())
                                {{ $canceledRequests->links() }}
                            @else
                                Showing {{ $canceledRequests->firstItem() }} to {{ $canceledRequests->lastItem() }} out of {{ $canceledRequests->total() }}
                                results
                            @endif
                        @else
