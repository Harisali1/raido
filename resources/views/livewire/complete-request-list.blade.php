                    ...
                    <div class="portlet-body">
                        ...
                        @if(count($completeRequests) > 0)
                            <table class="table table-striped table-bordered responsive table-hover table-checkable order-column"
                                ...
                                <tbody>
                                @foreach($completeRequests as $key => $userRequest)
                                    <tr class="odd gradeX">
                                        <td> {{ $key +1 }}</td>
                            ...
                            @if($completeRequests->total() > $completeRequests->perPage())
                                {{ $completeRequests->links() }}
                            @else
                                Showing {{ $completeRequests->firstItem() }} to {{ $completeRequests->lastItem() }} out of {{ $completeRequests->total() }}
                                results
                            @endif
                        @else
