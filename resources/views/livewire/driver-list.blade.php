                        ...
                        @if(count($drivers) > 0)
                            <table class="table table-striped table-bordered responsive table-hover table-checkable order-column"
                                ...
                                @foreach($drivers as $key => $driver)
                                    <tr class="odd gradeX">
                                        <td>{{ $key +1 }}</td>
                                        <td> <a href="{{route('admin.driver.show', $driver->id)}}">{{ $driver->name }}</a></td>
                                        <td>{{ $driver->email }} </td>
                                        <td>{{ $driver->mobile }}</td>
                                        <td>{{ optional($driver->vendor)->name ?? 'N/A' }}</td>
                                        <td>
                                            <input type="checkbox" class="make-switch" style="width: 100%; height: 100%; z-index: 999;"
                                                   ...
                                                   onclick="updateDriverStatus({{ $driver->id }})">
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.driver.vehicle.images.index', $driver->vehicle_id) }}">
                                            ...
                                            <button type="button" onclick="deleteDriver({{ $driver->id }})"
